...
public class Dcm2Jpg {
	...
	private static CommandLine parseComandLine(String[] args) throws ParseException {
		...
		opts.addOption(Option.builder("F").hasArg().argName("format").desc(rb.getString("format")).build());
		opts.addOption(Option.builder("E").hasArg().argName("class").desc(rb.getString("encoder")).build());
		opts.addOption(Option.builder("C").hasArg().argName("type").desc(rb.getString("compression")).build());
		opts.addOption(Option.builder("q").hasArg().argName("quality").type(PatternOptionBuilder.NUMBER_VALUE)
				.desc(rb.getString("quality")).build());
		opts.addOption(
				Option.builder().hasArg().argName("suffix").desc(rb.getString("suffix")).longOpt("suffix").build());
		opts.addOption(Option.builder().hasArg().argName("number").type(PatternOptionBuilder.NUMBER_VALUE)
				.desc(rb.getString("frame")).longOpt("frame").build());
		opts.addOption(Option.builder("c").hasArg().argName("center").type(PatternOptionBuilder.NUMBER_VALUE)
				.desc(rb.getString("windowCenter")).longOpt("windowCenter").build());
		opts.addOption(Option.builder("w").hasArg().argName("width").type(PatternOptionBuilder.NUMBER_VALUE)
				.desc(rb.getString("windowWidth")).longOpt("windowWidth").build());
		opts.addOption(Option.builder().hasArg().argName("number").type(PatternOptionBuilder.NUMBER_VALUE)
				.desc(rb.getString("window")).longOpt("window").build());
		opts.addOption(Option.builder().hasArg().argName("number").type(PatternOptionBuilder.NUMBER_VALUE)
				.desc(rb.getString("voilut")).longOpt("voilut").build());
		opts.addOption(Option.builder().hasArg().argName("file").type(PatternOptionBuilder.EXISTING_FILE_VALUE)
				.desc(rb.getString("ps")).longOpt("ps").build());
		opts.addOption(
				Option.builder().hasArg().argName("mask").desc(rb.getString("overlays")).longOpt("overlays").build());
		opts.addOption(
				Option.builder().hasArg().argName("value").desc(rb.getString("ovlygray")).longOpt("ovlygray").build());
	...
	public static void main(String[] args) {
		try {
			...
			main.initImageWriter(cl.getOptionValue("F", "JPEG"), cl.getOptionValue("suffix"), cl.getOptionValue("E"),
					cl.getOptionValue("C"), (Number) cl.getParsedOptionValue("q"));
	...
	public void convert(File src, int i) throws IOException {
		ImageInputStream iis = ImageIO.createImageInputStream(src);

		try {
			BufferedImage bi = readImage(i, iis);
			bi = convert(bi);
			ImageOutputStream ios = ImageIO.createImageOutputStream(new File("Img" + i + ".jpeg"));
			try {
				writeImage(ios, bi);
			} finally {
				try {
	...
	public void convert2(File src, File dest) throws Exception {
		final ImageInputStream iis = ImageIO.createImageInputStream(src);
		try {
			final DicomInputStream dis = new DicomInputStream(src);
			dis.setDicomInputHandler(new StopTagInputHandler(Tag.PixelData));
			final Attributes dicomObject = dis.readDataset(-1, Tag.PixelData);
			...
			// temp2[2] = 8196;
			// temp2[3] = 8196;
			// final StreamSegmentMapper itemParser = new SectorStreamSegmentMapper(temp,
			// 8196,0);
			// final StreamSegmentMapper itemParser = new StreamSegmentMapperImpl(temp,
			// temp2);
			final ItemParser itemParser = new ItemParser(dis, iis, 1, UID.MPEG2);
			final SegmentedImageInputStream siis = new SegmentedImageInputStream(iis, itemParser);

			itemParser.seekFrame(siis, 0);

			final File newFile = new File(src.getAbsoluteFile().getParent(),
					dicomObject.getString(Tag.SOPInstanceUID) + ".mpg");
			newFile.createNewFile();

			final FileOutputStream fos = new FileOutputStream(newFile);
			try {
				transfer(new ImageInputStreamWrapper(siis), fos);
			} finally {
				fos.close();
			}
		} finally {
	...
	public static void transfer(ImageInputStreamWrapper in, FileOutputStream out) throws Exception {
	...
	private BufferedImage readImage(Integer frame, ImageInputStream iis) throws IOException {
